...
mapp = [
  {
    ...
    text: "Saturday Night Market",
    time: "9 min walk",
  },
  {
    icon: "fa-solid fa-location-dot",
    text: "Baga Night Market",
    time: "13 min walk",
  },
...
btn.addEventListener("click", function () {
  ...
  noOfdays = Math.abs(
    checkOutDate.getTime() / day - checkInDate.getTime() / day
  );
...
function checkAvailability() {
  ...
  if (noOfTravellers.value > 2 * noOfroom.value) {
    ...
    p1.innerText =
      "The available rooms can't accommodate " +
      noOfTravellers.value +
      " guests. Add another room.";
...
function chooseRoom(roomData) {
  roomData.forEach((ele) => {
    ...
    btn15.addEventListener("click", function () {
      ...
      let z =
        (ele.price + variablePrice) *
        Number(noOfroom.value) *
        localStorage.getItem("noOfDays");
    ...
    document.querySelector("#check").addEventListener("click", function () {
      ...
      if (noOfroom.value > 1) {
        ...
        document.querySelector("#menu>button>a").innerText =
          "Reserve " + noOfroom.value + " Rooms";
      } else {
